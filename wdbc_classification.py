...
def load_wdbc_data(filename):
    ...
    with open(filename) as f:
        for line in f.readlines():
            ...
            wdbc.data.append([float(val) for val in items[2:]])
...
if __name__ == '__main__':
    ...
    for (x, y) in [(0, 1)]: # Not mandatory, but try [(i, i+1) for i in range(0, 30, 2)] -- plots all the attributes against each other
        plt.figure()
